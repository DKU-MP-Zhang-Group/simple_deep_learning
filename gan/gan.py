...
parser.add_argument("--n_epochs", "-e", type=int, default=50, help="number of epochs of training")
...
parser.add_argument("--n_cpu", "-n", type=int, default=2, help="number of cpu threads to use during batch generation")
...
grad_scaler_G = torch.cuda.amp.GradScaler(enabled=opt.amp)
grad_scaler_D = torch.cuda.amp.GradScaler(enabled=opt.amp)
## 如果有显卡，都在cuda模式中运行
...
for epoch in range(opt.n_epochs):                               ## epoch:50
    for i, (imgs, _) in enumerate(dataloader):                  ## imgs:(64, 1, 28, 28)     _:label(64)
        ...
        grad_scaler_D.scale(loss_D).backward()              ## 将误差反向传播
        grad_scaler_D.step(optimizer_D)                                  ## 更新参数
        ...
        for k in range(3):
            ...
            grad_scaler_G.scale(loss_G).backward()                                                   ## 进行反向传播
            grad_scaler_G.step(optimizer_G)                                         ## step()一般用在反向传播后面,用于更新生成网络的参数
        ...
        if i % 1000 == 0:
            print(
                "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f] [D real: %f] [D fake: %f] [G fake: %f]"
                % (epoch, opt.n_epochs, i, len(dataloader), loss_D.item(), loss_G.item(), sigmoid(real_scores.data.mean()), sigmoid(fake_scores.data.mean()), sigmoid(output.data.mean()))
            )
    ...
    if epoch % 100 == 0:   
        ## 保存模型
        torch.save(generator.state_dict(), './save/gan/generator_{:0>2d}.pth'.format(epoch))
        torch.save(discriminator.state_dict(), './save/gan/discriminator_{:0>2d}.pth'.format(epoch))
